...
async function prompt_user() {
  const questions = [
      ...
      : {
        ...
        validate: async (input) => {
          if (!/^[a-zA-Z0-9-_]+$/.test(input)) {
            return chalk.red('Le nom du projet ne doit contenir que des lettres, chiffres, tirets ou underscores.');
          }
          const exists = await fs
            .access(path.join(process.cwd(), input))
            .then(() => true)
            .catch(() => false);
          return exists ? chalk.red(`Le dossier "${input}" existe déjà.`) : true;
        }
      },
    {
      type: 'list',
    ...
    },
    {
      type: 'confirm',
      name: 'include_assets',
      message: chalk.cyan('🖼️ Voulez-vous inclure un dossier pour les images (public/assets/images) ?'),
      default: false
    },
    {
      type: 'confirm',
      name: 'include_pages',
      message: chalk.cyan('📄 Voulez-vous utiliser plusieurs pages HTML (public/pages) ?'),
      default: false
    },
    {
      type: 'confirm',
